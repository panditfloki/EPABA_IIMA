...
# 1. Load the data and Activate the libraries
...
# 2a. Forecasting the occupancy for breakfast

bfts<-ts(bf[4:115], start=c(1), end=c(16), frequency=7)

plot(bfts, xlab="Weeks", ylab="Occupancy for Breakfast", col='blue')

    ## Investigating for trend in the data through simple moving averages MA(7)

bfSMA7<-SMA(bf,n=7)
bfSMA7ts<-ts(bfSMA7, start=c(1), end=c(17), frequency=7)
plot(bfSMA7ts, xlab="Weeks", ylab="Occupancy for Breakfast", col='blue')

      ### Does not show trend

   ## Investigation for Seasonality in the data

ggseasonplot(bfts, col=rainbow(7), year.labels=TRUE)
 
     ### Seasonal plot does not indicate seasonality


   ## 2a1. Applying Single Exponential Smoothening for forecast

fitbfts<-ses(bfts,h=7)
plot(fitbfts)
summary(fitbfts)

fcst.fitbfts<-forecast(fitbfts)

SE.fitbfts<-(bfts-fcst.fitbfts$fitted)^2
MSE.fitbfts<-sum(SE.fitbfts)/106
RMSE.fitbfts<-(MSE.fitbfts)^0.5


      ### Forecasted Value = 254.4685 MSE = 220.9917

   ## 2a2. Applying SES with alpha=0.2, initial seed = Y(1)

fitbfts1<-ses(bfts,h=7, initial="simple", alpha=0.2)
plot(fitbfts1)
summary(fitbfts1)

fcst.fitbfts1<-forecast(fitbfts1)

SE.fitbfts1<-(bfts-fcst.fitbfts1$fitted)^2
MSE.fitbfts1<-sum(SE.fitbfts1)/106
RMSE.fitbfts1<-(MSE.fitbfts1)^0.5

     ### Forecasted Value = 238.2296 MSE = 311.5814

   ## 2a3. Double Exponential Smoothening (Holt)
fitbftsde<-holt(bfts,h=7)
plot(fitbftsde)  
summary(fitbftsde)

fcst.fitbftsde<-forecast(fitbftsde)

SE.fitbftsde<-(bfts-fcst.fitbftsde$fitted)^2
MSE.fitbftsde<-sum(SE.fitbftsde)/106
RMSE.fitbftsde<-(MSE.fitbftsde)^0.5

### Forecasted Value = 254.4333 MSE = 221.7538

  ## 2a.4 Triple Exponential Smoothening (Holt-Winter)
fitbftste<-hw(bfts, h=7)
plot(fitbftste)
summary(fitbftste)

fcst.fitbftste<-forecast(fitbftste)

SE.fitbftste<-(bfts-fcst.fitbftste$fitted)^2
MSE.fitbftste<-sum(SE.fitbftste)/106
RMSE.fitbftste<-(MSE.fitbftste)^0.5

     ### Forecasted Value = 254.2232 MSE = 158.9385


  ## 2a.5 Decomposition method with linear trend
t<-c(1:112)
...
MSE.bflint<-sum(SE.bflint)/112
  ...
  ## 2a.6 Decomposition method using STL
stlfit<-stl(bfts,t.window=15, s.window="periodic", robust=TRUE)
plot(stlfit)
summary(stlfit)
...
fcast.stlfit$fitted
SE.stlfit<-(bfts[2:105]-fcast.stlfit$fitted[2:105])^2
MSE.stlfit<-sum(SE.stlfit)/105

  ## 2a.7 Ensembled Forecast
...
## 3a1. Applying Single Exponential Smoothening for forecast
...
## 3a2. Applying SES with alpha=0.2, initial seed = Y(1)
...
## 3a3. Double Exponential Smoothening (Holt)
...
## 3a.4 Triple Exponential Smoothening (Holt-Winter)
...
## 3a.5 Decomposition method with linear trend
t<-c(1:112)
xq<-c("Th", "F", "Sa", "S", "M", "T", "W")
dwk<-rep(xq,16)
idlyt<-idly[4:115]
...
## 3a.6 Decomposition method using STL
...
## 4a1. Applying Single Exponential Smoothening for forecast
...
## 4a2. Applying SES with alpha=0.2, initial seed = Y(1)
...
## 4a3. Double Exponential Smoothening (Holt)
...
## 4a.4 Triple Exponential Smoothening (Holt-Winter)
...
## 4a.5 Decomposition method with linear trend
t<-c(1:112)
xq<-c("Th", "F", "Sa", "S", "M", "T", "W")
dwk<-rep(xq,16)
dosat<-dosa[4:115]
...
## 4a.6 Decomposition method using STL
...
## 5a1. Applying Single Exponential Smoothening for forecast
...
## 5a2. Applying SES with alpha=0.2, initial seed = Y(1)
...
## 5a3. Double Exponential Smoothening (Holt)
...
## 5a.4 Triple Exponential Smoothening (Holt-Winter)
...
## 5a.5 Decomposition method with linear trend
t<-c(1:112)
xq<-c("Th", "F", "Sa", "S", "M", "T", "W")
dwk<-rep(xq,16)
cht<-ch[4:115]
...
## 5a.6 Decomposition method using STL
...
## 6a1. Applying Single Exponential Smoothening for forecast
...
## 6a3. Double Exponential Smoothening (Holt)
...
## 6a.4 Triple Exponential Smoothening (Holt-Winter)
...
## 6a.5 Decomposition method with linear trend
t<-c(1:112)
xq<-c("Th", "F", "Sa", "S", "M", "T", "W")
dwk<-rep(xq,16)
smt<-sm[4:115]
...
## 6a.6 Decomposition method using STL
